...
1. 최초 진입점을 통해 인증 토큰 A를 발급 받는다.
2. A토큰을 들고 사용자 인증을 받아 B 토큰을 발급 받는다.
3. B토큰은 시스템 내에서 매 요청마다 Header에 실어 보내 자동으로 인증받도록 한다.
4. 인증 과정에서 토큰이 만료되었으면 공통 핸들러에서 재 발급을 받거나 최초 인증과정으로 보내버린다.
